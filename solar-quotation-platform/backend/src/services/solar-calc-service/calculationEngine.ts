...
export interface SolarCalcInputs {
  ...
  efficiency: number; // decimal, e.g. 0.75
  costPerWatt: number; // ₹/W
  installationMultiplier: number; // e.g. 0.1 = 10%
  gstRate: number; // decimal, e.g. 0.18 = 18%
 ...
 */
export function runSolarCalculation(
  ...
  const systemCapacity_kW =
    dailyConsumption / (irradiance_kWh_m2_day * efficiency);
  ...
  const annualGeneration_kWh =
    systemCapacity_kW * irradiance_kWh_m2_day * 365 * efficiency;
  ...
  const paybackPeriod_years =
    annualSavings > 0 ? totalSystemCost / annualSavings : 0;
