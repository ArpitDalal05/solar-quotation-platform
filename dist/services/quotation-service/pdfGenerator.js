...
async function generateQuotationPdf(data) {
    ...
    const logoPath = path_1.default.join(process.cwd(), "..", "frontend", "src", "assets", "logo.jpg");
    ...
    const drawSpecSection = (title, items, description, x, y, width) => {
    ...
    drawSpecSection("1. SYSTEM OVERVIEW", [`Capacity: ${cap.toFixed(2)} kWp`, "Type: Grid-Tied (On-Grid)"], "kWp represents the peak power output under standard test conditions.", 40, currentY, 250);
    drawSpecSection("2. SOLAR PANELS", [`Quantity: ${panels} Nos.`, "Type: Mono-PERC", "Durability: IP68 Rated"], "High-efficiency panels that capture more sunlight even in low-light conditions.", 305, currentY, 250);
    currentY += 75;
    drawSpecSection("3. INVERTER DETAILS", [`Capacity: ${cap.toFixed(2)} kW`, "Pure Sine Wave", "App-Based Monitoring"], "Converts DC to AC with 98% efficiency and tracks performance via your phone.", 40, currentY, 250);
    drawSpecSection("4. ENERGY GENERATION", [`Estimated: ${Math.round(gen).toLocaleString()} kWh/yr`, "Monthly: ~304 Units"], `This system can power approx. 4 ceiling fans, 1 fridge, and all LED lights daily.`, 305, currentY, 250);
    currentY += 85;
    drawSpecSection("5. MOUNTING STRUCTURE", ["Galvanized Iron (HDGI)", "Wind Speed: 150 km/h", "Corrosion Resistant"], "Engineered to withstand heavy winds and extreme weather for 25+ years.", 40, currentY, 250);
    drawSpecSection("6. ELECTRICAL & SAFETY", ["DC/AC Distribution Boxes", "Lightning Arrester", "Dual Earthing System"], "Complete protection against voltage surges and lightning strikes.", 305, currentY, 250);
